...
@override_settings(API_THROTTLE_ANON_RATE="1/min", API_THROTTLE_USER_RATE=None)
def test_public_api_middleware_does_not_resolve_user_for_off_api_request() -> None:
    request = RequestFactory().get("/admin/")
    request.user = _lazy_user_that_fails_if_resolved()
    middleware = PublicAPIThrottleMiddleware(
        lambda _request: HttpResponse(status=HTTPStatus.OK),
    )
...
@override_settings(API_THROTTLE_ANON_RATE="1/min", API_THROTTLE_USER_RATE=None)
def test_public_api_middleware_does_not_resolve_user_for_options_request() -> None:
    request = RequestFactory().options("/api/v1/notes")
    request.user = _lazy_user_that_fails_if_resolved()
    middleware = PublicAPIThrottleMiddleware(
        lambda _request: HttpResponse(status=HTTPStatus.OK),
    )
...
@override_settings(API_THROTTLE_ANON_RATE=None, API_THROTTLE_USER_RATE=None)
def test_public_api_middleware_does_not_resolve_user_when_rate_is_unset() -> None:
    request = RequestFactory().get("/api/v1/notes")
    request.user = _lazy_user_that_fails_if_resolved()
    middleware = PublicAPIThrottleMiddleware(
        lambda _request: HttpResponse(status=HTTPStatus.OK),
...
def _fail_if_user_resolved() -> AnonymousUser:
    pytest.fail("request.user must remain unresolved")  # pragma: no cover


def _lazy_user_that_fails_if_resolved() -> AnonymousUser:
    return cast("AnonymousUser", SimpleLazyObject(_fail_if_user_resolved))
